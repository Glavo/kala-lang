...
可用 `this-type` 软关键字（暂定名称）引用的特殊类型。该类型表示该值被使用处的静态类型。
...
否则： 
    {
        与属性相同的可访问性修饰符 与属性相同的静态或抽象修饰符 field;
...
* 字段声明（字段名不可为 `field`）：默认被翻译为类中名称为 `属性名$字段名` 的成员。
* 方法声明：默认被翻译为类中名称为 `属性名$方法名` 的成员。
* `field` 声明：使用软关键字 `field` 声明启用属性的后备字段，默认字段名称与属性名相同。`field` 前可以使用访问性修饰符修饰，也可以可选的明确声明其类型，默认类型与属性类型相同。`field` 可以被修饰为 `final` 的。
